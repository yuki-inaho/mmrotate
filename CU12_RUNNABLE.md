...
patch. Validated stack: **Python 3.10 / torch 2.1.0+cu121 / mmcv 2.1.0 /
mmengine 0.10.7 / mmdet 3.3.0 / mmrotate 1.0.0rc1**.

## Quick start (uv)

```bash
just sync          # uv sync (cu12 deps incl. mmcv 2.1.0 cu121 wheel, mmdet 3.3.0) + source on path (.pth)
just smoke         # import mmrotate (mmcv<=2.1.0 / mmdet<3.4.0 / mmengine<1.0 guards)
...
comes from the mmcv 2.1.0 cu121 wheel. mmrotate is pure Python, installed via `.pth`.
