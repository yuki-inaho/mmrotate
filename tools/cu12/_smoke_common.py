...
CUDA op and either returns a dict ``{"cuda_op_device": str, ...}`` on success
or raises. This helper records that result with torch / cuda / device metadata.
...
def run_smoke(
    *,
    ...
    op_device = ((payload['op_result'] or {}).get('cuda_op_device')
                 if payload['op_result'] else None)
    torch_ok = (payload['torch'] or {}).get('is_available') is True
    payload['cuda_op_device'] = op_device
    payload['success'] = ((op_device == 'cuda') and torch_ok
    ...
    summary = {
        ...
        op_device,
        'error_type':
        ((payload['error'] or {}).get('type') if payload['error'] else None),
    }
    if payload['error']:
