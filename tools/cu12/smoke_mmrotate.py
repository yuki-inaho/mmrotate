...
Exercises a rotated CUDA op. Because mmrotate fork main is 0.x with an
`mmcv-full` dependency, this script attempts ``mmcv.ops.box_iou_rotated``
as the rotated CUDA op (mmrotate >=1.x re-exports this from mmcv), and also
records the installed mmrotate version.
"""
...
def main() -> int:
    ...
    return run_smoke(module_name="mmrotate", runner=runner, output_json=args.output_json)
