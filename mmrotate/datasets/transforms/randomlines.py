...
This transform creates random line occlusion patterns to improve model
robustness. The lines can have various angles, thicknesses, and colors to
simulate real-world occlusions.
"""
...
class RandomLines(BaseTransform):
    ...
    This transform creates random lines across the image with varying angles,
        thicknesses, and colors to simulate occlusion effects and improve model
        robustness.
    ...
    Args:
        ...
        line_length_ratio (float or tuple): Ratio of line length to image
            diagonal. If tuple, randomly select from range. Default:
            (0.1, 0.8).
        line_opacity (float or tuple): Opacity of lines (0.0 to 1.0). If tuple,
            randomly select from range. Default: (0.3, 0.9).
        line_colors (list): List of possible RGB colors for lines. Random
            color will be selected from this list. Default:
            [(0, 0, 0), (255, 255, 255)].
        angle_range (tuple): Range of line angles in degrees. Default:
            (0, 180).
    ...
    def __init__(self,
                 ...
                 line_length_ratio: Union[float, Tuple[float,
                                                       float]] = (0.1, 0.8),
                 line_opacity: Union[float, Tuple[float, float]] = (0.3, 0.9),
                 line_colors: List[Tuple[int, int, int]] = [(0, 0, 0),
                                                            (255, 255, 255)],
    ...
    def _get_random_value(
            self, param: Union[int, float, Tuple]) -> Union[int, float]:
    ...
    def _generate_random_line(self, img_shape: Tuple[int,
                                                     int]) -> Dict[str, Any]:
    ...
    def _draw_line_with_opacity(self, img: np.ndarray,
                                line_params: Dict[str, Any]) -> np.ndarray:
        ...
        # Draw line on mask
        cv2.line(mask, line_params['start_point'], line_params['end_point'],
                 255, line_params['thickness'])
        ...
        for c in range(img.shape[2]):
            result[line_mask, c] = ((1 - opacity) * img[line_mask, c] +
                                    opacity * color[c]).astype(np.uint8)
