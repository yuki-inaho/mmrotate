...
This transform creates fence-like occlusion patterns to improve model
robustness. The fence pattern consists of vertical or horizontal lines that
...
class FenceMask(BaseTransform):
    ...
    Args:
        fence_direction (str): Direction of fence lines. Either 'vertical'
            or 'horizontal'. Default: 'vertical'.
        line_width (int or tuple): Width of fence lines in pixels. If tuple,
            randomly select from range. Default: (2, 8).
        line_spacing (int or tuple): Spacing between fence lines in pixels.
            If tuple, randomly select from range. Default: (20, 60).
        fence_opacity (float or tuple): Opacity of fence lines (0.0 to 1.0).
            If tuple, randomly select from range. Default: (0.3, 0.8).
        fence_color (tuple): RGB color of fence lines. Default: (0, 0, 0)
            for black.
    ...
    def __init__(self,
                 ...
                 coverage_ratio: Union[float, Tuple[float,
                                                    float]] = (0.1, 0.4),
                 prob: float = 0.5):
        ...
        assert fence_direction in [
            'vertical', 'horizontal'
        ], ("fence_direction must be 'vertical' or 'horizontal', "
            f'got {fence_direction}')
    ...
    def _apply_fence_mask(self, img: np.ndarray) -> np.ndarray:
        ...
        for c in range(img.shape[2]):
            result[fence_mask,
                   c] = ((1 - fence_opacity) * img[fence_mask, c] +
                         fence_opacity * self.fence_color[c]).astype(np.uint8)
